...
def draw_bounding_box_on_image_array(image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  draw_bounding_box_on_image(image_pil,img_name, ymin, xmin, ymax, xmax, color,
...
def draw_bounding_box_on_image(image,
  ...
  for display_str in display_str_list[::-1]:
    ...
    punc = '''!()-[]{};'%"\, <>./?@#$^&*~'''
    a = str(display_str_list)
    for i in a:
        if i in punc:
            a = a.replace(i, '')
    label, conf  = a.split(':')
    obj_info = img_name, label, int(conf)/100, int(left), int(top), int(right), int(bottom)
    all_data.append(obj_info)
    # df = pd.DataFrame(all_data,columns=['img', 'label', 'score', 'xmin', 'ymin', 'xmax', 'ymax'])
    # df.to_csv('/content/drive/My Drive/shelfer/data.csv',index=None)
...
def draw_bounding_boxes_on_image(image,
  ...
  for i in range(boxes_shape[0]):
    ...
    draw_bounding_box_on_image(image, img_name, boxes[i, 0], boxes[i, 1], boxes[i, 2],
